...
from .schemas import MasterclassMetaCreate
...
def get_masterclass_by_id(masterclass_id: UUID):
    with engine.begin() as conn:
...
def get_masterclasses_by_user(user_id: UUID):
    with engine.begin() as conn:
...
def get_masterclass_meta_by_id(meta_id: int):
    try:
        with engine.begin() as conn:
